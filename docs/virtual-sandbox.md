...
1. Install Percona XtraDB Cluster on three cluster nodes, as described in [Configuring Percona XtraDB Cluster on CentOS](configure-cluster-rhel.md#configure-a-cluster-on-red-hat-based-distributions).

2. On the client node, install [ProxySQL](load-balance-proxysql.md) and `sysbench`:
...
* [Configuring Percona XtraDB Cluster on CentOS](configure-cluster-rhel.md)
