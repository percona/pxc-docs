# Percona XtraDB Cluster 9.7.1-1 (2026-09-)
...
Percona XtraDB Cluster 9.7.1-1 is based on Percona Server for MySQL and includes all improvements and bug fixes available in the [Percona Server for MySQL 9.7.1-1 (2026-08-05) release notes](https://docs.percona.com/percona-server/9.7/release-notes/9.7.1-1.html).
...
* [PXC-5195](https://perconadev.atlassian.net/browse/PXC-5195): Removes obsolete configuration variables (`wsrep-slave-FK-checks`, `wsrep-slave-UK-checks`, `wsrep-causal-reads`) from Percona XtraDB Cluster 9.7.1-1.
...
* [PXC-5233](https://perconadev.atlassian.net/browse/PXC-5233): Fixed an issue where the Percona XtraDB Cluster 9.7 entrypoint invoked the removed `mysql_upgrade` binary, causing an unexpected exit loop during rolling upgrades from Percona XtraDB Cluster 8.4.
...
* [PXC-5230](https://perconadev.atlassian.net/browse/PXC-5230): Fixed potentially dangling pointer existence at the end of `sst_joiner_thread()`.

* [PXC-5282](https://perconadev.atlassian.net/browse/PXC-5282): Fixed an issue where upgrading from Percona XtraDB Cluster 8.4 to 9.7 did not fully update the `mysql.pxc.sst.role` table, which could cause clone State Snapshot Transfer (SST) operations to fail.

* PXC-5209: Fixed an issue where a corrupted Galera cache could cause the server to exit during startup.

* [PXC-5244](https://perconadev.atlassian.net/browse/PXC-5244): Changes the default XtraBackup locking mode to `lock-ddl=ON` during State Snapshot Transfer (SST).
...
* In Percona XtraDB Cluster 9.7.1-1 only, packaging was reorganized to align more closely with upstream MySQL, consistent with the equivalent change in Percona Server for MySQL 9.7.1-1. Several packages were split into separate components, which may affect upgrades and dependency resolution compared with earlier Percona XtraDB Cluster releases.
...
* The [Percona XtraDB Cluster installation instructions](https://www.percona.com/doc/percona-xtradb-cluster/9.7/install/index.html)
...
* To contribute to the documentation, review the [Documentation Contribution Guide](https://github.com/percona/pxc-docs/blob/9.7/contributing.md)
