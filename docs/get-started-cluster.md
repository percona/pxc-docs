...
* [Install Percona XtraDB Cluster](install-index.md) on all nodes and set up root access for them.

  It is recommended to install from official Percona repositories:

  * On Red Hat and CentOS, [install using YUM](yum.md#install-percona-xtradb-cluster-on-red-hat-enterprise-linux-and-centos).

  * On Debian and Ubuntu, [install using APT](apt.md#install-percona-xtradb-cluster-on-debian-or-ubuntu).
