...
* [Install Percona XtraDB Cluster](install-index.md#install-percona-xtradb-cluster) on all nodes and set up root access for them.

  It is recommended to install from official Percona repositories:

  * On Red Hat and CentOS, [install using YUM](yum.md#install-from-percona-software-repository).

  * On Debian and Ubuntu, [install using APT](apt.md#install-from-repository).

* [Configure all nodes](configure-nodes.md#configure-nodes-for-write-set-replication) with relevant settings required for write-set replication.

  This includes path to the Galera library, location of other nodes, etc.

* [Bootstrap the first node](bootstrap.md#bootstrap-the-first-node) to initialize the cluster.

  This must be the node with your main database,
  which will be used as the data source for the cluster.

* [Add other nodes](add-node.md#add-nodes-to-cluster) to the cluster.

  Data on new nodes joining the cluster is overwritten
  in order to synchronize it with the cluster.

* [Verify replication](verify-replication.md#verify-replication).
