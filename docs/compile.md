...
XtraBackup archives stamp glibc into the filename; treat `glibc2.xx` as a placeholder and substitute your real suffix (for example `glibc2.28`). Pick tarballs whose glibc matches what you compile and run. Check the host with:
...
3. Download Percona XtraBackup {{vers}} and Percona XtraBackup 8.4 `*.tar.gz` bundles for your OS from [Percona Software Downloads :octicons-link-external-16:](https://www.percona.com/downloads/).
    ...
    The repository does not ship XtraBackup; `build-binary.sh` copies it out of `pxc-build/pxc_extra`. It looks for two separate install trees with fixed names—`pxb-{{vers_major}}` powers backup and restore, `pxb-8.4` powers State Snapshot Transfer (SST):

    * Percona XtraBackup {{vers}} backs up and restores data through the integrated tooling.

    * Percona XtraBackup 8.4 drives SST.

    You need two directories on disk, not always two different releases. When one tarball satisfies both roles (same version, glibc, and CPU), unpack once, copy the folder into the second `pxb-*` path, or run the extract twice on the same file.
    ...
    Stage everything under `pxc-build/pxc_extra`, then rename to match the `mv` targets in the snippet. Map `glibc2.xx` and `8.4.x` to the strings in your real filenames (for example `8.4.0-3`). Aim each `tar` and `mv` at the top-level folder that unpack creates.
