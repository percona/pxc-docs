...
**NOTE**: The Percona Server for MySQL 5.7 data at rest encryption is similar to
...
plugin. [Install the plugin](https://dev.mysql.com/doc/refman/5.7/en/install-plugin.html) and add the following options in the configuration file:
...
Prior to Percona XtraDB Cluster 

```
:rn:`5.7.22-29.26`
```

 the DONOR node had to send the
...
, a node will fail to start if it fails to load keyring plugin.
...
**NOTE**: Percona XtraDB Cluster 5.6 does not have encrypted tablespaces. No major
...
**NOTE**: MySQL 5.7.21 supports [migration between keystores](https://dev.mysql.com/doc/mysql-security-excerpt/5.7/en/keyring-key-migration.html). Migration requires a restart.
