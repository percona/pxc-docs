...
* `gcache.page`
    ...
    See [`gcache.page_size`](wsrep-provider-index.md#gcache.page_size)
...
* `galera.cache`
    ...
    File size can be controlled with the variable [`gcache.size`](wsrep-provider-index.md#gcache.size). If
    this value is bigger, more writesets are cached and chances are better that
    the re-joining node will get [IST](glossary.md#IST) instead of [SST](glossary.md#term-SST). Filename can be changed
    with the [`gcache.name`](wsrep-provider-index.md#gcache.name) variable.
...
* `grastate.dat`
    ...
    * `uuid` - a unique identifier for the state and the sequence of changes it undergoes.For more information on how UUID is generated see [UUID](glossary.md#term-UUID).
...
* `gvwstate.dat`
   ...
   First part contains the node [UUID](glossary.md#term-UUID) information. Second part contains
