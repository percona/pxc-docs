...
2. Replication traffic, that includes [SST](../glossary.md#term-SST), [IST](../glossary.md#term-IST), write-set
...
Generating Keys and Certificates Manually section.

## Encrypting Replication Traffic

Replication traffic refers to the inter-node traffic which includes
[SST](../glossary.md#term-SST) traffic, [IST](../glossary.md#term-IST) traffic, and replication traffic.
...
Percona XtraDB Cluster includes the 
...
[SST](../glossary.md#term-SST), [IST](../glossary.md#term-IST), and replication traffic.
...
the following settings in `my.cnf` configuration file:
...
manually created ones, as covered by the Generating Keys and Certificates Manually section.
...
necessary files, see Generating Keys and Certificates Manually.
...
This refers to [SST](../glossary.md#term-SST) traffic during full data copy
...
such as, write-set replication, [IST](../glossary.md#term-IST), and various service messages.
...
This is the default SST method (the 
...
```
 ...
 is set
to `xtrabackup-v2`), which uses [Percona XtraBackup](https://www.percona.com/software/mysql-database/percona-xtrabackup) to perform non-blocking transfer
...
For more information, see Generating Keys and Certificates Manually.
...
due to the [logjam vulnerability](https://en.wikipedia.org/wiki/Logjam_(computer_security)).
...
configured for Encrypting Client-Server Communication,
...
3. To make sure that the SST user replicated across the cluster,
...
4. Specify corresponding certificate files
...
5. Also specify the SST user credentials
in the 
...
* Incremental State Transfer ([IST](../glossary.md#term-IST)) which
...
(`gcomm`). Securing this channel will ensure that [IST](../glossary.md#term-IST) traffic,
...
preferably those used for Encrypting Client-Server Communication.
...
**NOTE**: Upgrading Certificates subsection covers the details on upgrading
...
1. Restart the first node with the 
...
2. Restart the second node with the 
...
3. Restart the first node with the new certificate files,
