...
By default, Percona XtraDB Cluster uses [Percona XtraBackup](https://www.percona.com/software/mysql-database/percona-xtrabackup) for *State Snapshot Transfer* ([SST](glossary.md#term-SST)).
...
Specify authentication credentials for [SST](glossary.md#term-SST) as `<sstuser>:<sst_pass>`.
...
[PXC Strict Mode](features/pxc-strict-mode.md#pxc-strict-mode) is enabled by default and set to `ENFORCING`, which blocks the use of experimental and unsupported features in Percona XtraDB Cluster.

[`binlog_format`](http://dev.mysql.com/doc/refman/5.7/en/replication-options-binary-log.html#sysvar_binlog_format)

Galera supports only row-level replication, so set `binlog_format=ROW`.

[`default_storage_engine`](http://dev.mysql.com/doc/refman/5.7/en/server-system-variables.html#sysvar_default_storage_engine)
...
[`innodb_autoinc_lock_mode`](http://dev.mysql.com/doc/refman/5.7/en/innodb-parameters.html#sysvar_innodb_autoinc_lock_mode)
