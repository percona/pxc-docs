...
For the `option httpchk` configuration shown in [Configure HAProxy with `clustercheck` health checks](#configure-haproxy-with-clustercheck-health-checks-recommended), HAProxy checks
