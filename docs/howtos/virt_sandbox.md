...
3. Make sure that the `my.cnf` configuration file on the first node
...
5. Adjust the `my.cnf` configuration files
...
[SST](../glossary.md#term-SST) is performed by taking a backup using XtraBackup,
then copying it to the new node with `netcat`.
After a successful [SST](../glossary.md#term-SST), you should see the following in the error log:
...
For debugging information about the [SST](../glossary.md#term-SST),
you can check the `sst.err` file and the error log.

After [SST](../glossary.md#term-SST) finishes, you can check the cluster size as follows:
...
If you are using [HAProxy](../glossary.md#term-HAProxy) for *MySQL* you can break the privilege system’s
...
[HAProxy](../glossary.md#term-HAProxy) instance, each application server connects to 127.0.0.1, so MySQL
