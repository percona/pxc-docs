...
This manual requires three virtual machines for Percona XtraDB Cluster nodes,
...
1. Install Percona XtraDB Cluster on the three cluster nodes, as described in [Installing Percona XtraDB Cluster on Red Hat Enterprise Linux and CentOS](../install/yum.md#yum).
...
6. Start the second and third nodes.
    ...
    [SST](../glossary.md#sst) is performed by taking a backup using XtraBackup,
    then copying it to the new node with `netcat`.
    After a successful [SST](../glossary.md#sst), you should see the following in the error log:
    ...
    For debugging information about the [SST](../glossary.md#sst),
    you can check the `sst.err` file and the error log.

    After [SST](../glossary.md#sst) finishes, you can check the cluster size as follows:
...
7. When all cluster nodes are started, configure HAProxy on the client node.
   This will enable the application to connect to localhost
   as if it were a single MySQL server, instead of a Percona XtraDB Cluster node.
    ...
    * The `pxc-onenode` pair is for connecting to one node at a time
    ...
    !!! note
        ...
        and checks the node via the `wsrep_local_state` variable.
...
After you set up Percona XtraDB Cluster in a sand box, you can test it using
...
4. Run the same benchmark on port 3306:
    ...
    If you are using [HAProxy](../glossary.md#haproxy) for *MySQL* you can break the privilege system’s host part, because *MySQL* will think that the connections are always coming from the load balancer. You can work this around using T-Proxy patches and some iptables magic for the backwards connections. However in the setup described in this how-to this is not an issue, since each application server has it’s own [HAProxy](../glossary.md#haproxy) instance, each application server connects to 127.0.0.1, so MySQL will see that connections are coming from the application servers. Just like in the normal case.
