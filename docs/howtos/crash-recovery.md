...
joins the cluster based on its 
...
If the writeset cache (

```
:variable:`gcache.size`
```

) on nodes B and/or C still
has all the transactions executed while node A was down, joining is possible via
[IST](../glossary.md#term-IST). If [IST](../glossary.md#term-IST) is impossible due to missing transactions in donor’s
gcache, the fallback decision is made by the donor and [SST](../glossary.md#term-SST) is started
automatically.

## Scenario: Two nodes are gracefully stopped

Similar to Scenario: Node A is gracefully stopped, the cluster size is reduced to
...
perform the full [SST](../glossary.md#term-SST) to join the cluster initialized from the less
...
**NOTE**: Even though you bootstrap from the most advanced node, the other
nodes have a lower sequence number. They will still have to join via the full [SST](../glossary.md#term-SST)
...
**NOTE**: After a shutdown, you can boostrap from the node which is marked as safe in the
`grastate.dat` file.
...
In recent Galera versions, the option 
...
again using the same command as described in Scenario: Two nodes disappear from the cluster
...
and the other half should be able to automatically re-join using [IST](../glossary.md#term-IST) as
soon as the network link is restored.

**WARNING**: If you set the bootstrap option on both the separated parts, you will end up
