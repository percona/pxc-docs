...
5. Back up `grastate.dat`, so that you can restore it
...
**NOTE**: As of Percona XtraDB Cluster 5.7.6, the `--skip-grant-tables` option
is not required.
...
**NOTE**: On CentOS, the `my.cnf` configuration file
...
in the in the *MySQL* configuration file, `my.cnf`.
...
10. Repeat this procedure for the next node in the cluster
until you upgrade all nodes.

It is important that on rejoining, the node should synchronize using
[IST](../glossary.md#term-IST). For this, it is best not to leave the cluster node being
upgraded offline for an extended period. More on this below.

When performing any upgrade (major or minor), [SST](../glossary.md#term-SST) could
be initiated by the joiner node after the upgrade if the server
was offline for some time. After [SST](../glossary.md#term-SST) completes, the data
...
> **NOTE**: In case of [SST](../glossary.md#term-SST) synchronization, the error log
...
> IST: …” lines appropriate to  [IST](../glossary.md#term-IST)
> synchronization.
...
directory structure after [SST](../glossary.md#term-SST) (after the normal major or
minor upgrade steps):


1. shutdown the node that rejoined the cluster using [SST](../glossary.md#term-SST):
...
2. restart the node in standalone mode by setting the
...
4. restart the node in cluster mode (e.g by executing `sudo service mysql
start` and make sure the cluster joins back using [IST](../glossary.md#term-IST).
