...
**NOTE**: Before using the `proxysql-admin` tool, ensure that ProxySQL and
Percona XtraDB Cluster nodes you want to add are running. For security purposes,
...
change default ProxySQL credentials
...
**WARNING**: Running more then one copy of `proxysql_galera_check` in the same runtime
