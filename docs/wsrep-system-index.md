...
Controls [PXC Strict Mode](features/pxc-strict-mode.md#pxc-strict-mode), which runs validations
...
or the node is bootstrapping, then 
...
By default, it uses the IP address from 
...
```
 ...
 is an example where you may only use
the value of 
...
If you want to desync a node, use 
...
For older nodes in the cluster, 
...
```
:variable:`wsrep_replicate_miysam`
```
...
```
 ...
 as suggested [in the Galera Cluster documentation](https://galeracluster.com/library/training/tutorials/galera-monitoring.html).
...
Specifies a list of nodes (using their 
...
```
 ...
 values)
that the current node should prefer as donors for [SST](glossary.md#term-SST) and [IST](glossary.md#term-IST).

**WARNING**: Using IP addresses of nodes instead of node names (the value of
...
This is the **recommended** and default method for Percona XtraDB Cluster.
...
meaning that the IP address from 
...
```
 ...
 to `1` is the equivalent
of setting the deprecated 
