...
2. Configure password storage with Docker Secrets or `.env`.
    ...
    === "Option B: .env (quick local test only)"
...
Create `docker-compose.yml` in the project root. The following example uses Docker Secrets (Option A in [Configuration files](#configuration-files)). If you chose the `.env` option (Option B), make these changes:
...
1. Start the bootstrap node to initialize the cluster:
...
2. Wait for pxc1 to report healthy before starting pxc2 and pxc3. The bootstrap node must accept connections, or the other nodes can fail to join. Use a wait loop:
