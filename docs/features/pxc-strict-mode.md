...
allowed: the validation of explicit table locking is not performed.  This mode can be used with
...
MyISAM replication is controlled using the 
...
```
 ...
 variable controls *replication* for
...
operation and doesn’t result in an error.
     ...
     |
**NOTE**: Setting *binlog_format* at global level to STATEMENT/MIXED is not allowed under
...
**NOTE**: This type of validation is not applicable to temporary tables, system tables
...
Percona XtraDB Cluster does not support tables in the MySQL database as the destination for log
output.  By default, log entries are written to a file.  This validation checks
the value of the [`log_output`](http://dev.mysql.com/doc/refman/5.7/en/server-system-variables.html#sysvar_log_output)  variable.
...
**NOTE**: This validation is not performed during runtime, because the
...
| ENFORCING
                            ...
                            | No validation is performed.
                                                                                                                                   ...
                                                                                                                                   |
**NOTE**: CTAS operations for temporary tables are permitted even in strict mode.
