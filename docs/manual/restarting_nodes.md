...
(and the total vote count for [quorum](../glossary.md#term-quorum) should decrement).

When it rejoins, the node should synchronize using [IST](../glossary.md#term-IST).
If the set of changes needed for IST are not found in the `gcache` file
on any other node in the entire cluster,
then [SST](../glossary.md#term-SST) will be performed instead.
