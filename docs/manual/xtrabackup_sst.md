...
based on the presence of `xtrabackup_ist` file on the joiner node.


* Then it starts data transfer:


    * In case of [SST](../glossary.md#term-SST), it empties the data directory except for some files
(`galera.cache`, `sst_in_progress`, `grastate.dat`)
and then proceeds with SST


    * In case of [IST](../glossary.md#term-IST), it proceeds as before.

**NOTE**: As of Percona XtraDB Cluster 5.7, `xtrabackup-v2` is the only XtraBackup SST method.

## SST Options

The following options specific to [SST](../glossary.md#term-SST) can be used in `my.cnf`
under `[sst]`.
...
* SST script reads `my.cnf` when it runs
...
(values `2`, `3`, and `4` for the `encrypt` option).
...
* **Example**
    ...
    tcert=/etc/ssl/certs/mycert.pem
...
see [http://www.dest-unreach.org/socat/doc/socat-openssltunnel.html](http://www.dest-unreach.org/socat/doc/socat-openssltunnel.html).
...
[Galera cluster](https://galeracluster.com/library/documentation/ssl-config.html)
...
**NOTE**: For versions of Percona XtraDB Cluster before 5.7.17-29.20,
...
**NOTE**: You can also enable SSL based compression with `sockopt`.
...
**NOTE**: [#1192834](https://bugs.launchpad.net/percona-xtradb-cluster/+bug/1192834) affects this option.
...
* **Default**
    ...
    `'.\*\\.pem$\\|.\*init\\.ok$\\|.\*galera\\.cache$\\|.\*sst_in_progress$\\|.\*\\.sst$\\|.\*gvwstate\\.dat$\\|.\*grastate\\.dat$\\|.\*\\.err$\\|.\*\\.log$\\|.\*RPM_UPGRADE_MARKER$\\|.\*RPM_UPGRADE_HISTORY$'`


Used to define the files
that need to be retained in the [datadir](../glossary.md#term-datadir) before running SST,
...
they cannot be specified in `my.cnf`,
...
**NOTE**: Starting from Percona XtraDB Cluster 
...
[here](http://www.percona.com/doc/percona-xtrabackup/2.4/innobackupex/encrypted_backups_innobackupex.html).
...
don’t provide any XtraBackup encryption options in `my.cnf`.

**NOTE**: The `encrypt` option under `[sst]` is different
...
it should be specified as `encrypt-algo`.
