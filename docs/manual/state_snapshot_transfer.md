...
Xtrabackup SST, on the other hand, uses [backup locks](http://www.percona.com/doc/percona-server/5.7/management/backup_locks.html),
...
that can safely perform incremental state transfer ([IST](../glossary.md#term-IST)),
the cluster defaults to [SST](../glossary.md#term-SST).

If there are nodes available that can perform [IST](../glossary.md#term-IST),
the cluster prefers a local node over remote nodes to serve as the donor.

If there are no local nodes available that can perform [IST](../glossary.md#term-IST),
the cluster chooses a remote node to serve as the donor.

If there are several local and remote nodes that can perform [IST](../glossary.md#term-IST),
...
This is the least blocking method that leverages [backup locks](http://www.percona.com/doc/percona-server/5.7/management/backup_locks.html).
...
```
 ...
 variable.
Besides the credentials, the [datadir](../glossary.md#term-datadir) needs to be specified
...
This method is the slowest and it performs a global lock during [SST](../glossary.md#term-SST),
