...
[IST](glossary.md#ist). If [IST](glossary.md#ist) is impossible due to missing transactions in donor’s gcache, the fallback decision is made by the donor and [SST](glossary.md#sst) is started automatically.
...
!!! note
    ...
    Even though you bootstrap from the most advanced node, the other nodes have a lower sequence number. They will still have to join via the full [SST](glossary.md#sst) because the *Galera Cache* is not retained on restart.
