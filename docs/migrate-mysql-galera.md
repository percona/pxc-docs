...
Percona XtraDB Cluster (PXC) uses the same [write-set](glossary.md#write-set) replication model as MySQL Galera Cluster, so a migration replaces the binaries and keeps the [datadir](glossary.md#datadir). The schemas, data, user accounts, and privileges carry over.

Starting in PXC 9.7.2, a PXC node can join a running MySQL Galera Cluster. You can convert nodes one at a time, in place, so the cluster stays available throughout and no extra hardware is required.
...
Both approaches keep the cluster available. PXC 9.7.1 cannot form a mixed cluster with Galera Cluster, so a migration to that version needs a short full-cluster outage.

!!! important "Set these before you start a PXC node"

    * Use `wsrep_sst_method=xtrabackup-v2`. PXC requires this SST method.

    * PXC encrypts the Galera channel by default. Set [`pxc_encrypt_cluster_traffic`](encrypt-traffic.md#encrypt-pxc-traffic) so it matches the Galera Cluster before you start the PXC node.
...
* PXC packages for your platform. See [Install on Debian or Ubuntu](apt.md) or [Install on Red Hat Enterprise Linux](yum.md).

* Root or `sudo` access on every node.

Reuse the Galera Cluster `my.cnf`. Keep [`wsrep`](glossary.md#wsrep) cluster identity the same (`wsrep_cluster_name`, `wsrep_cluster_address`, and the per-node `wsrep_node_name` and `wsrep_node_address`), then change only what PXC requires:

* `wsrep_sst_method=xtrabackup-v2`. PXC uses Percona XtraBackup for SST. Do not carry over `rsync`, MariaBackup, or a custom SST script. See [Percona XtraBackup SST configuration](xtrabackup-sst.md).

* `wsrep_provider` must point at the PXC Galera library. See [Configure nodes for write-set replication](configure-nodes.md).

* [`pxc_encrypt_cluster_traffic`](encrypt-traffic.md#encrypt-pxc-traffic) must match the Galera Cluster. PXC enables it by default.
...
## Migrate with PXC 9.7.2 or later

Introduced in Percona XtraDB Cluster 9.7.2, you can join a running Galera Cluster. Choose either scenario. Both keep the cluster available throughout.
...
2. Install PXC on that host. Keep the Galera Cluster `my.cnf`, apply the adjustments in [Before you start](#before-you-start), and start the node. It joins the cluster with IST.
...
## Migrate with PXC 9.7.1

PXC 9.7.1 cannot form a cluster with Galera Cluster nodes, so you bootstrap a new cluster from an existing datadir during a maintenance window.
{.power-number}
...
| SST fails | Unsupported SST method | `wsrep_sst_method` must be `xtrabackup-v2`. See [`wsrep_sst_allowed_methods`](wsrep-system-index.md#wsrep_sst_allowed_methods) and [XtraBackup SST configuration](xtrabackup-sst.md). |
...
| Cluster size does not recover | [Quorum](glossary.md#quorum) loss | See [Cluster failover](failover.md). |
